/*
  Arduino With ESP8266 Controller

  This Sketch is for controlling some outputs of arduino by a direct command or
  by setting a timer for each output using a mobile application designed to communicate
  with arduino using ESP8266 module.
  ...
  Some parts of this Sketch is based on this tutorial code:
...
*/
//TODO app - DEBUG command - check alarm ID from app
//TODO add battery level on screen 
//TODO add power saving mode
//TODO Debug Level
...
/***Commands**/
#define Set "SET"
#define Add "ADD" // Request string from mobile application to know the state of each output
#define Del "DEL" // Request string from mobile application to make an output on
#define Sync "SYN" // Request string from mobile application to make an output off
#define Debug "DEB"
#define ResetMem "RES"
...
void setup()
{
  ...
  LoadArraies(); //load processing arries with values readed from EEPROM
...
void loop()
{
  ...
  if (esp8266_Available()) // check if the esp is sending a message
  {
    if (esp8266_Find("+IPD,"))
    {
      ...
      esp8266_Find('?'); // This caracter define the start of command in the our message body
      ...
      else if (InStream.equals(Add) || InStream.equals(Del)) // ADD,ID,A(active)/D(deactive),HHMM,RepDays,SoundType,Title
      {
        ...
        if (Alarm_AD == 'A')
        {
          ...
          int HEX_val;
          if ((Alarm_Rep + 48) == 'O' || (Alarm_Rep + 48) == 'F') // O - no repeat , F - all days , ex: 123 means repeat alarm on Mon+Tue+Wed
          {
